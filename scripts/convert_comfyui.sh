...
run_or_skip "Stage 3: ComfyUI FP8 quantization" "$FP8_FILE" \
...
run_or_skip "Stage 5: ComfyUI NVFP4 quantization (Blackwell)" "$NVFP4_FILE" \
    python3 /scripts/quantize_nvfp4.py "$COMFYUI_FILE" "$NVFP4_FILE"

run_or_skip "Stage 6: ComfyUI MXFP8 quantization (Blackwell)" "$MXFP8_FILE" \
...
echo ""
echo "File sizes:"
for f in "$MERGED_FILE" "$COMFYUI_FILE" "$FP8_FILE" "$INT8_FILE" "$NVFP4_FILE" "$MXFP8_FILE"; do
    if [ -f "$f" ]; then
